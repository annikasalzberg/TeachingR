...
```{r, warning=FALSE}
plot(Plumbers$Year, Plumbers$count, main="Plumbers in Paris", xlab="Years ", ylab="# of Plumbers")
abline(lm(Plumbers$count~Plumbers$Year))
```
...
There are a ton of these, but we'll just go over a few of the most common ones.
...
```{r}
Plumbers$month <- factor(Plumbers$month, 
  levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun",
             "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))
```

This is a manual fix for when you can't automatically change the levels of a column. We can use the arrange() function for this:
```{r, eval=FALSE}
rearrangedVector <- arrange(data, variable1, variable2, variable3)
```
But if our levels are characters that the program doesn't automatically recognize how to order (like in our example of the months above), we have to do it the manual way.
...
```{r}
plumberBasic + 
  ylab('Number of Plumbers') +
  xlab('Years') +
  ggtitle('Plumber Population of Paris')
```
...
```
However, the "bin" (or "count") functionality is being phased out of ggplot - and you should really use the geom_histogram function instead if you want to graph count data! The point of the above is to show why it's important that you take note of which values your graph is actually showing.
